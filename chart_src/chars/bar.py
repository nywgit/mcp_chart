...
class BarChart:
    ...
    def get_chart_image(self, df, storage_tool):
        ...
        :return: 结果字典，包含柱状图 URL 或 Base64 字符串
        """
        self.plt.figure(figsize=(10, 6))
        #bar_width = 0.35
        # 根据类别数量动态调整柱子宽度
        n_categories = len(self.y_label_list)
        bar_width = 0.8 / n_categories  # 总宽度0.8，均分给每个类别
        gap = 0.1  # 年份之间的间隙

        index = range(len(df))
        for i, item_y_label in enumerate(self.y_label_list):
            y_col_name = f'y{i}'  # 对应y0, y1, y2等列
            offset = (i - n_categories / 2 + 0.5) * bar_width  # 计算偏移量，使柱子居中
            self.plt.bar([pos + offset for pos in index], df[y_col_name], width=bar_width, label=item_y_label, edgecolor='white', linewidth=1)

        self.plt.title(self.title)
        self.plt.xlabel(self.x_label)
        self.plt.ylabel(self.y_label)
        self.plt.xticks(index, df['x'])
        # ✅ 正确做法：只对数值列求最大值
        ...
        self.plt.legend()
        self.plt.grid(axis='y', linestyle='--', alpha=0.6)
